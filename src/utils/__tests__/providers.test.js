...
describe('Provider', () => {
  ...
  it('returns a npm path with a version', async () => {
    const provider = new Provider('https://www.npmjs.com/package/async/v/2.6.0')
    const result = await provider.get()
    expect(result).toEqual('npm/npmjs/-/async/2.6.0')
  })
  it('returns a github path with a version', async () => {
    const provider = new Provider('https://github.com/github/fetch/releases/tag/v3.0.0')
    const result = await provider.get()
    expect(result).toEqual('git/github/github/fetch/v3.0.0')
  })
  it('returns a github path with a version', async () => {
    const provider = new Provider('https://github.com/github/fetch/commit/d4ed806fdcbdeaef707d27f6c88943f0336a647d')
    const result = await provider.get()
